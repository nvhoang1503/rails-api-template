module UserBase
  ...
  included do
    ...
    before_save :ensure_users_roles_updated
  end

  def ensure_users_roles_updated
    self.try(:users_role).try(:save)
  end
  ...
  # note: authentication_token is ensured to be not nil
  # def authentication_token=(authentication_token)
  #   if new_record?
  #     self.add_role(self.class.name.downcase.to_sym)
  #     self.users_roles.last.authentication_token = authentication_token if self.try(:users_roles).try(:last)
  #   else
  #     self.users_role.authentication_token = authentication_token
  #   end
  # end
  ...
  def new_chat_notif_count
    users_role.try(:new_chat_notif_count)
  end

  def reset_new_sys_notif_count!
    users_role.try(:reset_new_sys_notif_count!)
  end

  def reset_new_chat_notif_count!
    users_role.try(:reset_new_chat_notif_count!)
  end
